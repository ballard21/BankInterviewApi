...
namespace Interview.Api.Services
{
    public interface ITransactionService
    {
        ...
        Task<Tuple<string, object>> InsertTransaction(TransactionDto txn);
    }
    public class TransactionService : ITransactionService
    {
        ...
        public async Task<Tuple<string, object>> InsertTransaction(TransactionDto txn)
        {
            try
            {
                ...
                if (acc == null)
                    return new Tuple<string, object>("failed", new { Message = "Invalid phone number" });
                ...
                if (contact == null || contact.CanTransact == false)
                    return new Tuple<string, object>("failed", new { Message = "Phone number not whitelisted" });
                ...
                return new Tuple<string, object>(status, new { TranId = tbTxn.Id, Message = "Transaction Successful" });
            }
            catch (Exception ex)
            {
                return new Tuple<string, object>("Failed", new { TranId = -1, Message = "Something went wrong "+ex.Message });
            }
